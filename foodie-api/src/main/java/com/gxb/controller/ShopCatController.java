...
public class ShopCatController {
    ...
    @ApiOperation(value = "从购物车中删除商品",notes = "从购物车中删除商品",tags = {"从购物车中删除商品"},httpMethod = "POST")
    @PostMapping("del")
    public JSONResult del(@RequestParam("userId") String userId,
                          @RequestParam("itemSpecId") String itemSpecId) {
        if (StringUtils.isBlank(userId) || StringUtils.isBlank(itemSpecId)) {
            return JSONResult.errorMsg("参数不能为空");
        }
        //TODO 用户在页面购物车中的商品数据，如果此时用户已经登录，则需要同步删除后端购物车中的商品
